...
def test_default_state_row_count():
    ...
    state: DataFrame = game_state_updates.create_default_state(players)
...
def test_update_state_for_cards():
    ...
    cards = ['2s', '4c', '9h', 'ad']
    state: DataFrame = game_state_updates.create_default_state(players)
    state = game_state_updates.update_state_upon_receiving_cards(state, 'a', cards)
...
def test_declaration_check():
    ...
    state: DataFrame = game_state_updates.create_default_state(players)
    state = game_state_updates.update_state_upon_receiving_cards(state, 'a', cards_for_a)
    state = game_state_updates.update_state_upon_receiving_cards(state, 'b', cards_for_b)
    state = game_state_updates.update_state_upon_receiving_cards(state, 'c', cards_for_c)

    # check declaration
    opt_declared_map = game_state_updates.able_to_declare(state, players, CardSet.LOW_SPADES)
...
def test_process_of_elimination():
    ...
    state = game_state_updates.process_of_elimination(state, 'ah')
    print(state)
...
def test_players_out_of_cards():
    ...
    state = game_state_updates.update_state_upon_receiving_cards(state, 'b', cards_for_b)
    state = game_state_updates.update_state_upon_receiving_cards(state, 'c', cards_for_c)
    state = game_state_updates.update_state_upon_receiving_cards(state, 'd', cards_for_d)
    question = Question('c', 'd', '7s', True)
    state = game_state_updates.update_state_with_turn(state, question)

    players = game_state_updates.get_players_out_of_cards(state)
    assert 'd' in players
